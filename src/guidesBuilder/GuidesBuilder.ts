 ...
 * 
 * GuidesBuilder строит направляющие множества для правил грамматики.
 * Направляющие множества (Guide sets) помогают LL-парсеру определить,
 * какое правило применять при встрече определенного токена.
 * 
 * Основные задачи:
 * 1. Построение отношения FIRST для нетерминалов
 * 2. Построение отношения FOLLOW для нетерминалов
 * 3. Создание правил с направляющими множествами
 * 4. Обработка транзитивного замыкания для распространения зависимостей
 * 
 * В результате работы для каждого нетерминала и каждой его альтернативы
 * создается направляющее множество, которое содержит терминальные символы,
 * по которым можно однозначно определить, какое правило применять.
 */
