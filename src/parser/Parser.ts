 ...
 * 
 * Parser - это рекурсивный нисходящий синтаксический анализатор, 
 * который проверяет соответствие входной строки заданной грамматике.
 * 
 * Основные компоненты:
 * - Лексический анализатор (Lexer) для получения токенов
 * - Набор методов для распознавания различных синтаксических конструкций
 * - Механизм обработки ошибок
 * 
 * Этот парсер реализует рекурсивный нисходящий алгоритм, который
 * рекурсивно применяет правила грамматики, начиная с верхнего уровня (выражения),
 * и спускается вниз до терминальных символов.
 * 
 * В отличие от LL-парсера, этот рекурсивный нисходящий парсер
 * не использует таблицу для принятия решений, а полагается на
 * прямую реализацию правил грамматики в виде методов.
 */
export class Parser {
  private lexer: Lexer;
